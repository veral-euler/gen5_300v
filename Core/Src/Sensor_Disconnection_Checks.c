...
uint8_t encoder_ab_error_check(void) {
  ...
  if (d.A_Pulse == GPIO_PIN_SET && d.B_Pulse == GPIO_PIN_SET && d.A_B_XOR == GPIO_PIN_RESET && fabsf(FOC_MTPA_FF_Y.Iq_ref) >= 250.0f && fabsf(FOC_MTPA_FF_Y.Id_ref) >= 250.0f && d.count_diff_at_z >= 60) {
    return !HAL_OK; // A and B pulses are the same, which is an error
  } else if (d.A_Pulse == GPIO_PIN_SET && fabsf(FOC_MTPA_FF_Y.Iq_ref) >= 250.0f && fabsf(FOC_MTPA_FF_Y.Id_ref) >= 250.0f && d.count_diff_at_z >= 60) {
    return !HAL_OK; // A pulse is set but B pulse is not, which is an error
  } else if (d.B_Pulse == GPIO_PIN_SET && fabsf(FOC_MTPA_FF_Y.Iq_ref) >= 250.0f && fabsf(FOC_MTPA_FF_Y.Id_ref) >= 250.0f && d.count_diff_at_z >= 60) {
...
uint8_t encoder_5v_error_check(void) {
  if (d.A_Pulse == GPIO_PIN_RESET && d.B_Pulse == GPIO_PIN_RESET && d.A_B_XOR == GPIO_PIN_RESET && HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_RESET && fabsf(FOC_MTPA_FF_Y.Id_ref) >= 250.0f && fabsf(FOC_MTPA_FF_Y.Iq_ref) >= 250.0f && d.count_diff_at_z >= 60) {
    return !HAL_OK;
  }
